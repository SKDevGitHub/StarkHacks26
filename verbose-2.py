...
# Edit this to match your hardware. Values can be:
#   int   → /dev/videoN or DirectShow index
#   str   → device path, RTSP URL, or file path
#
SOURCE_MAP: dict[str, int | str] = {
    "1": 0,          # /0/ and /0/whep  → camera device 0
    "0": 1,          # /1/ and /1/whep  → camera device 2  (Mac: second camera)
}

PERSON_CLASS = 0

# session_id → RTCPeerConnection  (across all cameras)
...
def get_buffer(request) -> YOLOFrameBuffer | None:
    """Resolve the camera index from the URL and return its buffer."""
...
async def whep_post(request):
    """
    WHEP POST — client sends SDP offer (application/sdp),
...
// Build nav links to sibling cameras
...
def main():
    ...
    # Build one buffer per entry in SOURCE_MAP
    ...
    # Routes — all parameterised on {cam_idx}
